...
class CDG_Theme
{
    ...
    private function init_components(): void
    {
        try {
            ...
            if (class_exists('CDG_Logger')) {
                $this->components['logger'] = CDG_Logger::get_instance();
            }
            ...
            if (isset($this->components['logger'])) {
                $this->components['logger']->info('CDG Theme initialized successfully', [
                    'divi_version' => $this->divi_version,
    ...
    private function init_divi_5_features(): void
    {
        ...
        add_filter('et_core_page_resource_hints', [$this, 'add_divi_5_resource_hints']);
    }
        ...
        try {
            ...
            if (isset($this->components['logger'])) {
                $this->components['logger']->error(
                    sprintf('Failed to initialize component %s', $name),
                    ['error' => $e->getMessage()]
                );
            }
            error_log(sprintf('CDG Theme: Failed to initialize %s - %s', $name, $e->getMessage()));
        }
    }
    ...
    public function render_admin_page(): void
    {
        ...
        <div class="wrap">
            ...
            <div class="card" style="max-width: 800px; padding: 20px; margin-top: 20px;">
                ...
                <?php
                $status_class = $this->initialized ? 'notice-success' : 'notice-warning';
                $status_text = $this->initialized
                    ? __('Active', 'cdg-custom')
                    : __('Limited Mode', 'cdg-custom');
                ?>
                
                <div class="notice inline <?php echo esc_attr($status_class); ?>" style="margin: 15px 0;">
                    <p>
                        <strong><?php esc_html_e('Status:', 'cdg-custom'); ?></strong>
                ...
                <table class="form-table" role="presentation">
                    <tbody>
                        <tr>
                            <th scope="row"><?php esc_html_e('Theme Version', 'cdg-custom'); ?></th>
                            <td><?php echo esc_html(CDG_THEME_VERSION); ?></td>
                        </tr>
                        <tr>
                            <th scope="row"><?php esc_html_e('Divi Version', 'cdg-custom'); ?></th>
                            <td><?php echo esc_html($this->divi_version ?? __('Unknown', 'cdg-custom')); ?></td>
                        </tr>
                        <tr>
                            <th scope="row"><?php esc_html_e('Divi 5 Support', 'cdg-custom'); ?></th>
                            <td>
                                <?php if ($this->is_divi_5): ?>
                                    <span class="dashicons dashicons-yes-alt" style="color: #00a32a;"></span>
                                    <?php esc_html_e('Enabled', 'cdg-custom'); ?>
                                <?php else: ?>
                                    <span class="dashicons dashicons-minus" style="color: #dba617;"></span>
                                    <?php esc_html_e('Divi 4 Mode', 'cdg-custom'); ?>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><?php esc_html_e('PHP Version', 'cdg-custom'); ?></th>
                            <td><?php echo esc_html(PHP_VERSION); ?></td>
                        </tr>
                        <tr>
                            <th scope="row"><?php esc_html_e('WordPress Version', 'cdg-custom'); ?></th>
                            <td><?php echo esc_html(get_bloginfo('version')); ?></td>
                        </tr>
                        <tr>
                            <th scope="row"><?php esc_html_e('CDG Core Plugin', 'cdg-custom'); ?></th>
                            <td>
                                <?php if (defined('CDG_CORE_VERSION')): ?>
                                    <span class="dashicons dashicons-yes-alt" style="color: #00a32a;"></span>
                                    <?php echo esc_html(sprintf(__('Active (v%s)', 'cdg-custom'), CDG_CORE_VERSION)); ?>
                                <?php else: ?>
                                    <span class="dashicons dashicons-warning" style="color: #d63638;"></span>
                                    <?php esc_html_e('Not Detected', 'cdg-custom'); ?>
                                    <br><small><?php esc_html_e('Install the CDG Core mu-plugin for full functionality.', 'cdg-custom'); ?></small>
                                <?php endif; ?>
                            </td>
                        </tr>
                    </tbody>
    ...
    public function check_compatibility(): void
    {
        ...
        // Check Divi version
        if ($this->divi_version && version_compare($this->divi_version, '4.0', '<')) {
            printf(
                '<div class="notice notice-warning"><p>%s</p></div>',
                esc_html__(
    ...
    {
        add_filter('et_builder_module_settings', [$this, 'customize_module_settings']);
    }
     ...
     * @return void
     */
    public function cleanup_on_deactivation(): void
    {
        wp_clear_scheduled_hook('cdg_cleanup_caches');
        ...
        if (isset($this->components['logger'])) {
            $this->components['logger']->info('CDG Theme deactivated');
        }
    }
